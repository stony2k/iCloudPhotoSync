...
def _resolve_target_dir(path, account_id=None):
    ...
    FileChooser returns virtual share paths:
      /home/Test     -> /volume1/homes/<user>/Test  (special: "home" share = "homes" dir)
      /photo/iCloud  -> /volume1/photo/iCloud
    ...
    # /home/... is a per-user virtual share -- maps to /volume1/homes/<user>/...
    if path.startswith("/home/") or path == "/home":
        sub = path[5:].lstrip("/")
        # Primary source: the DSM username stored on the account when the user
        # saved the config. The package user's home (set by data-share) must
        # never be used as a fallback — it silently hides files from the user.
        dsm_user = ""
        if account_id:
        ...
        # Unresolved: return a sentinel that the writability check will
        # reject AND that produces a clear error message (rather than
        # confusingly suggesting a permissions fix on a virtual /home/...
        # path that isn't a real filesystem location).
        return "__UNRESOLVED_HOME__:" + path

    # Other shared folders: prepend /volume1
    return "/volume1" + path
...
def _run_sync_locked(account_id):
    ...
    raw_target = sync_config.get("target_dir", "/volume1/iCloudPhotos")
    ...
    if isinstance(target_dir, str) and target_dir.startswith("__UNRESOLVED_HOME__:"):
        ...
        progress.error = (
            ...
            "(z.B. /volume1/iCloudPhotos)." % raw_target
        )
        progress.save()
        return progress
